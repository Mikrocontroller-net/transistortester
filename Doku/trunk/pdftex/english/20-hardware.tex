...
The Makefile option \inquotes{WITH\_LCD\_ST7565 = 7920} must be set in any case, for the serial
connection type you must set also the option \inquotes{CFLAGS += -DLCD\_INTERFACE\_MODE=5}.
...
like \inquotes{open collector} outputs with the Makefile option LCD\_SPI\_OPEN\_COL.
You must use \inquotes{Pull-Up} resistors with this option or you should not set
...
The missing pull-up resistor results to a switch off of the tester with the message \inquotes{Timeout}
...
I have analysed the board version \inquotes{2.1 2012/11/06}. One error is the assembly of the part \inquotes{D1},
...
Another user has problems with the same software version 1.05k. This user has the chinese board \inquotes{2.2 2012/11/26}.
...
but the internal \inquotes{pull-up} resistors are switched on during the output of a high level.
If the option PULLUP\_DISABLE is set, additionally a external pull-up resistor for
the \inquotes{Reset} signal (PD0) is required.
...
caused a \inquotes{Brown Out} reset of the processor.
...
A additional chinese version with graphical display is the \inquotes{LCD-T4} tester with a yellow 
printed board.
...
Also the activating of the \inquotes{brown-out} monitor by setting the extended fuse has not
shown any surprice.
...
Due to the fact, that the board can not be easy upgraded with a \inquotes{pull-up} resistor,
...
A additional chinese version with graphical display is offered with the name \inquotes{GM328}.
...
correct operation.
For correct operation of the \inquotes{GM328} tester with newer software versions you should cut the
...
\textbf {The test ports TP1, TP2 and TP3} are used for automatic component recognition
...
The \textbf {Testport TP2} is also used for the output of the special function \inquotes{f-Generator}.\\
