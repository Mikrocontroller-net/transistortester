...
Die LCD-Ausgabe von Test-Anschlussnummern wurde in der Form \inquotes{lcd\_data('1'+Pin)} durchgeführt.
Um den Platz für die zusätzliche Addition für jeden Aufruf zu sparen, wurde die
Funktion \inquotes{lcd\_testpin(uint8\_t pin)} in die Datei lcd\_routines.c eingefügt.
...
Die Routinen enthalten den \inquotes{Watch Dog Reset}-Befehl für alle Aufrufe über \(50ms\).
...
Zusätzlich wurde die oft benutzte Folge \inquotes{wait5ms(); ReadADC(\dots);} durch einen einzelnen Aufruf
\inquotes{W5msReadADC(\dots);} ersetzt.
Dasselbe wurde auch für die Folge \inquotes{wait20ms(); ReadADC(\dots);} gemacht, die durch den Aufruf
\inquotes{W20msReadADC(\dots);} ersetzt wurde.
