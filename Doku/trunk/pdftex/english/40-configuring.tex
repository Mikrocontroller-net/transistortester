...
the EEprom memory, the tool \lcmd{avrdude} (version 5.11svn) was taken by the Makefile, if you call ''make upload''.
...
\lcmd{avrdude} program mask the unused bits, so that it expect a ''0'' for all unused bits.
...
%  \begin{center}
    ...
    \lcmd{make fuses} & to set the ATmega ''fuses'' without crystali RC-clock\\
    \lcmd{make fuses-crystal} & to set the ATmega ,,fuses'' for crystal operation!\\
