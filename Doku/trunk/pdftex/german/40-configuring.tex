...
Programm \lcmd{avrdude} \cite{avrdude} (Version 5.11svn) von der \lname{Makefile} benutzt, wenn man ,,make upload'' aufruft.
...
Wenn Sie die \lname{Makefile} ändern, wird die Software komplett neu übersetzt, wenn man ,,make'' oder
,,make upload'' aufruft.
...
Wenn Sie meine \lname{Makefile} zusammen mit dem Programm \lcmd{avrdude} \cite{avrdude} benutzen, brauchen Sie
...
das \lcmd{avrdude} Programm maskiert die unbenutzten Bits, so daß es eine ,,0'' für alle unbenutzen Bits erwartet.
...
Arbeit erforderlich. Als PORT kann hier fest ,,usb'' in der \lname{Makefile}
...
\lcmd{make fuses}		& zum setzen von ATmega ,,fuses'' ohne Quarz\\
\lcmd{make fuses-crystal}	& zum Setzen von ATmega ,,fuses'' NUR mit Version mit Quarz!\\
