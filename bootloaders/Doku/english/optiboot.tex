...
Until now the functions of the Makefile is only tested with a Linux System and
...
\item [program\_target.sh] is called from build\_hex.sh, to check iand correct
...
\item [only\_avrdude.sh] makes only one avrdude call with the variable
DUDE\_PARAMS list of params.
For check of the parameters the call is recordes at the terminal.
...
at a Linux system.
For this tasks are matching chips available, which are most have a name beginning with
/dev/ttyUSB . There are also some software emulations available at mikrocontrollers,
which has a integrated USB ingterface. You cann access these Interfaces with
...
Script auch mit bash ./find\_serials aufrufen.
If at your terminal window  a bash command interpreter is running,
you can also set the execution bit for the file and call the script directly
with ./find\_serials.sh
