...
Um die Verzweiflungen und "`schlaflose Nächte"', die Schreiber dieses Unterkapitel erlitten hatte,
nach dem er, ohne jegliche AVR Erfahrung, einen Clonetester erworben hatte und diesem die
deutsche Sprache "`beibringen"' wollte, anderen Kollegen zu ersparen,
...
Dieses Handbuch auf einen USB-Stick kopieren und im Dateibrowser mit Doppelclick der
linken Maustaste \LMB öffnen.
Alternativ kann mit der \RMB Taste ein anderes als das
...
verschieben. Zum Verschieben kann man auch mit der \LMB Taste die
...
um ein Befehlsfenster zu öffnen.
Dies wird nun auf die gleiche Weise zur rechten Hälfte des Bildschirmes verschoben und
...
zusammen sammeln und erstickt nicht in Fenstern. 


\subsection{Programm Pakete installieren}

Nun brauchen wir einen Internetzugang.
Um den Tester zu programmieren, müssen zuerst Programmpakete
...
Dann den Text mit gedrückter linken Maustaste \LMB markieren,
die \LMB Taste wieder loslassen
und den Mauszeiger in das rechte Befehlsfenster führen.
Hier kann jetzt durch Drücken der mittlerer Maustaste (Scroll-Rad)
{weiter als \MMB abgekürzt} der vorher kopierte Text einfügt werden.
Nach dem Bestätigung mit \keys{\enter}, wird von 'sudo' noch das Benutzerpassword verlangt.
Anders als bei Windows wird das Passwort \textbf{Blind} (mit verdecktem Echo auf dem Bildschirm)
eingegeben und mit \keys{\enter} oder \keys{\return} bestätigt.
...
die man in der Regel mit [J] beantworten kann.
...
kann man mit dem folgendn Kommando prüfen:
\verb"git version"\\
...
Mit der Terminaleingabe \mbox{\keys{l} \keys{s} \keys{\return}} kann man das Vorhandensein überprüfen.
...
durch die Eingabe \mbox{man <kommando>} oder durch \mbox{<kommando> --help}.
...
Viele ISP-Programmer benutzen eine serielle Schnittstelle für
die Verbindung zum Rechner.
Für diese Geräte sollten Sie überprüfen, welche Gerätebezeichnung
diesem Gerät zugeordnet ist.
Alle modernen Programmer mit serieller Schnittstelle benutzen die USB Schnittstelle,
...
Systemprotokoll vorgenommen. Da das Systemprotokoll eine Textdatei ist,
...
\verb"dmesg|tail"\\
Für einen Pololu Programmer sieht das Ergebnis dass so aus:
\begin{footnotesize}
\begin{verbatim}
...
Das sind die Gerätebezeichnungen für zwei serielle Schnittstellen.
...
Jetzt muß für dieses Beispiel nur der PORT Eintrag in der Makefile auf
...
\subsection{Gruppen Mitgliedschaft}
...
Sollten Probleme auftreten kann man die Mitgliedschaft auch mit der graphischen Bedienoberfläche:
\menu[,]{Menü,Systemverwaltung,{Benutzer und Gruppen},{?Passwort}}.
Das Menü kann man auch mit der \keys{\winmenu} Taste zwischen \keys{{Strg}} und
\keys{\Alt} öffnen.
Nun erscheint ein Fenster mit zwei Reitern.
Wenn man nun im Reiter Benutzer auf seinen Name klickt,
sieht man rechts sein Profil und die Gruppen Zugehörigkeiten.
Mit dem Button \fbox{ADD} ist nun möglich neue Gruppen hinzufügen.
